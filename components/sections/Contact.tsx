...
export default function Contact() {
  ...
  useEffect(() => {
    async function fetchContactSettings() {
      ...
      if (data) {
        setSettings({
          contact_heading:
            data.contact_heading || fallbackSettings.contact_heading,
          contact_description:
            data.contact_description || fallbackSettings.contact_description,
          contact_button_label:
            data.contact_button_label || fallbackSettings.contact_button_label,
  ...
  if (!loading && settings.show_contact_section === false) {
    return null;
  }
    ...
    <section id="contact" className="relative py-24">
      <Container>
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          >
            {error && (
              <div className="mb-5 rounded-2xl border border-red-500 bg-red-500/10 p-4 text-red-300">
                {error}
              </div>
            )}

            {success && (
              <div className="mb-5 rounded-2xl border border-green-500 bg-green-500/10 p-4 text-green-300">
                {success}
              </div>
            )}
            ...
            <textarea
              ...
              placeholder="Message"
              rows={7}
              className="mt-5 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-white outline-none transition focus:border-white/40"
            />

            <button
              type="submit"
              disabled={sending}
              className="mt-6 rounded-full bg-white px-6 py-3 font-medium text-black transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-50"
            >
