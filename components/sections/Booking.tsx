...
function timeToMinutes(time: string) { const [hour, minute] = time.split(":"); return Number(hour) * 60 + Number(minute ?? "0"); }
function minutesToTime(minutes: number) { return `${String(Math.floor(minutes / 60) % 24).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`; }
function addMinutesToTime(time: string, minutesToAdd: number) { return minutesToTime(timeToMinutes(time) + minutesToAdd); }
function formatTime(time: string) { const [h, m] = time.split(":"); const hour = Number(h); const suffix = hour >= 12 ? "PM" : "AM"; return `${hour % 12 || 12}:${String(Number(m ?? 0)).padStart(2, "0")} ${suffix}`; }
...
function ruleMatches(rule: Row, context: { serviceId: number | null; variationId: number | null; email: string; servicePrice: number; total: number }) {
  const comparisonPrice = rule.price_basis === "total" ? context.total : context.servicePrice;
...
export default function Booking() {
  ...
  async function fetchBookingData() {
    try {
      ...
      setServices(data.services ?? []); setVariations(data.variations ?? []); setAvailability(data.availability ?? []); setExistingBookings(data.bookings ?? []); setAddons(data.addons ?? []); setAddonAssignments(data.addon_assignments ?? []); setDepositRules(data.deposit_rules ?? []);
    } catch (fetchError) {
  ...
  const paymentMode = normalizePaymentMode(matchedDepositRule?.payment_mode ?? "pay_later");
  const depositType = matchedDepositRule?.deposit_type ?? "amount";
  const depositValue = Number(matchedDepositRule?.deposit_value ?? 0);
  const tipAmount = selectedTip === "custom" ? roundMoney(Math.max(Number(customTip || 0), 0)) : roundMoney(Math.max(Number(selectedTip || 0), 0));
  ...
  const baseDueNow = roundMoney(calculateAmountDueNow({ discountedPrice, paymentMode, depositType, depositValue }));
  const amountDueNow = roundMoney(baseDueNow + tipAmount);
  const remainingBalance = paymentMode === "pay_later" ? discountedPrice : paymentMode === "deposit" ? roundMoney(Math.max(discountedPrice - baseDueNow, 0)) : 0;
  ...
  async function applyDiscountCode() {
    ...
    } catch (discountError) { console.error("DISCOUNT ERROR:", discountError); setDiscountMessage("Discount could not be applied."); }
  ...
  async function createCheckout() {
    try {
      ...
      if (!data.url) throw new Error("Booking did not return a redirect URL.");
      window.location.href = data.url;
    } catch (checkoutError) { console.error("CHECKOUT ERROR:", checkoutError); setError(checkoutError instanceof Error ? checkoutError.message : "Booking could not be created."); }
    finally { setCheckoutLoading(false); }
  }

  const paymentModeLabel = paymentMode === "pay_later" ? "Book Now, Pay Later" : paymentMode === "deposit" ? "Deposit Required" : "Full Payment Required";
  const primaryButtonLabel = checkoutLoading ? "Working..." : paymentMode === "pay_later" && amountDueNow <= 0 ? "Book Appointment" : `Pay ${formatMoney(amountDueNow)}`;
