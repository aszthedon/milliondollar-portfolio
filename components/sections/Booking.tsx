...
type DiscountResult = { valid: boolean; code: string; discount_amount: number; discounted_price: number; message: string };
...
function formatDateForDatabase(date: Date) { const year = date.getFullYear(); const month = String(date.getMonth() + 1).padStart(2, "0"); const day = String(date.getDate()).padStart(2, "0"); return `${year}-${month}-${day}`; }
function timeToMinutes(time: string) { const [hourString, minuteString] = time.split(":"); return Number(hourString) * 60 + Number(minuteString ?? "0"); }
...
function formatTime(time: string) { const [hourString, minuteString] = time.split(":"); const hour = Number(hourString); const minute = Number(minuteString ?? "0"); if (Number.isNaN(hour) || Number.isNaN(minute)) return time; const suffix = hour >= 12 ? "PM" : "AM"; const displayHour = hour % 12 || 12; return `${displayHour}:${String(minute).padStart(2, "0")} ${suffix}`; }
function roundMoney(value: number) { return Math.round(value * 100) / 100; }
function formatMoney(value: number) { return `$${roundMoney(value).toFixed(2)}`; }
function calculateDepositBaseAmount({ discountedPrice, paymentMode, depositType, depositValue }: { discountedPrice: number; paymentMode: string; depositType: string; depositValue: number }) { if (paymentMode !== "deposit") return discountedPrice; if (depositType === "amount") return Math.min(depositValue, discountedPrice); return Math.min((discountedPrice * depositValue) / 100, discountedPrice); }
function isCancelledStatus(status: string | null) { return status === "cancelled" || status === "rejected"; }

function ruleMatches(rule: Row, context: { serviceId: number | null; variationId: number | null; email: string; servicePrice: number; total: number }) {
...
export default function Booking() {
  ...
  async function fetchBookingData() {
    try { setLoading(true); setError(""); const response = await fetch("/api/public/booking-data", { cache: "no-store" }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error ?? "Booking information could not be loaded."); setServices(data.services ?? []); setVariations(data.variations ?? []); setAvailability(data.availability ?? []); setExistingBookings(data.bookings ?? []); setAddons(data.addons ?? []); setAddonAssignments(data.addon_assignments ?? []); setDepositRules(data.deposit_rules ?? []); } catch (fetchError) { console.error("BOOKING DATA FETCH ERROR:", fetchError); setError(fetchError instanceof Error ? fetchError.message : "Booking information could not be loaded."); } finally { setLoading(false); }
  }
  ...
  const availableAddons = useMemo(() => { if (!selectedService) return []; const enabledAddonIds = new Set(addonAssignments.filter((assignment) => Number(assignment.service_id) === Number(selectedService.id) && assignment.is_enabled !== false).map((assignment) => Number(assignment.addon_id))); return addons.filter((addon) => enabledAddonIds.has(Number(addon.id)) && addon.is_active !== false); }, [selectedService, addons, addonAssignments]);
  ...
  const matchedDepositRule = useMemo(() => depositRules.find((rule) => ruleMatches(rule, { serviceId: selectedService ? Number(selectedService.id) : null, variationId: selectedVariation ? Number(selectedVariation.id) : null, email: customerEmail.trim().toLowerCase(), servicePrice: activePrice, total: grossPrice })), [depositRules, selectedService, selectedVariation, customerEmail, activePrice, grossPrice]);
  ...
  const availableTimes = useMemo(() => { if (!selectedDateString || !activeOption) return []; const windowsForDate = availability.filter((window) => window.available_date === selectedDateString); const bookingsForDate = existingBookings.filter((booking) => booking.booking_date === selectedDateString && !isCancelledStatus(booking.status)); const generatedTimes: string[] = []; windowsForDate.forEach((window) => { const windowStart = window.start_time ?? window.available_time; const windowEnd = window.end_time; if (!windowStart) return; if (!windowEnd) { generatedTimes.push(windowStart); return; } let current = timeToMinutes(windowStart); const end = timeToMinutes(windowEnd); while (current + activeDuration <= end) { const candidateStart = minutesToTime(current); const candidateEnd = minutesToTime(current + activeDuration); const hasConflict = bookingsForDate.some((booking) => { if (!booking.booking_time) return false; const existingStart = timeToMinutes(booking.booking_time); const existingEnd = booking.booking_end_time ? timeToMinutes(booking.booking_end_time) : existingStart + 60; return timeToMinutes(candidateStart) < existingEnd && timeToMinutes(candidateEnd) > existingStart; }); if (!hasConflict) generatedTimes.push(candidateStart); current += 30; } }); return Array.from(new Set(generatedTimes)).sort((a, b) => timeToMinutes(a) - timeToMinutes(b)); }, [selectedDateString, activeOption, availability, existingBookings, activeDuration]);
  ...
  function tileDisabled({ date, view }: { date: Date; view: string }) { if (view !== "month") return false; const today = new Date(); today.setHours(0, 0, 0, 0); const targetDate = new Date(date); targetDate.setHours(0, 0, 0, 0); if (targetDate < today) return true; return !availableDates.has(formatDateForDatabase(date)); }
  function tileClassName({ date, view }: { date: Date; view: string }) { if (view !== "month") return ""; return availableDates.has(formatDateForDatabase(date)) ? "booking-calendar-available" : ""; }

  async function applyDiscountCode() { try { setDiscountLoading(true); setDiscountMessage(""); setAppliedDiscount(null); if (!discountCode.trim()) { setDiscountMessage("Enter a discount code first."); return; } if (!activeOption || grossPrice <= 0) { setDiscountMessage("Select a service before applying a discount."); return; } const response = await fetch("/api/discounts/validate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: discountCode, price: grossPrice }) }); const data = await response.json(); if (!response.ok || !data.valid) { setDiscountMessage(data.message || data.error || "Invalid code."); return; } setAppliedDiscount({ valid: true, code: data.code, discount_amount: Number(data.discount_amount ?? 0), discounted_price: Number(data.discounted_price ?? 0), message: data.message ?? "Discount applied." }); setDiscountMessage(data.message ?? "Discount applied."); } catch (discountError) { console.error("DISCOUNT ERROR:", discountError); setDiscountMessage("Discount could not be applied."); } finally { setDiscountLoading(false); } }
  async function createCheckout() { try { setCheckoutLoading(true); setError(""); if (!selectedService || !activeOption) { setError("Select a service first."); return; } if (!selectedDateString) { setError("Select a booking date."); return; } if (!selectedTime) { setError("Select a booking time."); return; } if (!customerEmail.trim()) { setError("Enter your email address."); return; } if (amountDueNow <= 0) { setError("Checkout amount must be greater than $0."); return; } const bookingEndTime = addMinutesToTime(selectedTime, activeDuration); const serviceName = selectedVariation ? `${selectedService.title} — ${selectedVariation.variation_name}` : selectedService.title; const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ service_id: selectedService.id, variation_id: selectedVariation?.id ?? null, addon_ids: selectedAddonIds, service_name: serviceName, price: activePrice, duration: activeDuration, customer_email: customerEmail.trim(), booking_date: selectedDateString, booking_time: selectedTime, booking_end_time: bookingEndTime, notes, timezone, client_id: null, discount_code: appliedDiscount?.code ?? "", tip_amount: tipAmount }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Checkout could not be created."); if (!data.url) throw new Error("Stripe did not return a checkout URL."); window.location.href = data.url; } catch (checkoutError) { console.error("CHECKOUT ERROR:", checkoutError); setError(checkoutError instanceof Error ? checkoutError.message : "Checkout could not be created."); } finally { setCheckoutLoading(false); } }

  if (loading) return <section id="booking" className="bg-black px-6 py-24 text-white md:px-10"><div className="mx-auto max-w-6xl rounded-3xl border border-white/10 bg-white/5 p-10 text-center"><p className="text-sm uppercase tracking-[0.3em] text-zinc-500">Loading</p><h2 className="mt-4 text-3xl font-bold">Loading Booking Calendar...</h2></div></section>;
...
function EmptyBox({ text }: { text: string }) { return <div className="rounded-2xl border border-dashed border-white/10 bg-black/40 p-6 text-zinc-500">{text}</div>; }
function SummaryRow({ label, value }: { label: string; value: string }) { return <div className="mb-3 flex items-center justify-between gap-4 text-sm"><span className="text-zinc-500">{label}</span><span className="font-bold text-white">{value}</span></div>; }
