...
export async function GET(request: Request) {
  try {
    ...
    const [sectionsResult, servicesResult, variationsResult, availabilityResult, bookingsResult, addonsResult, assignmentsResult, depositRulesResult] = await Promise.all([
    ...
    return NextResponse.json({ site_slug: siteSlug, sections: sectionsResult.data ?? [], services: servicesResult.data ?? [], variations: variationsResult.data ?? [], availability: availabilityResult.data ?? [], bookings: bookingsResult.data ?? [], addons: addonsResult.data ?? [], addon_assignments: assignmentsResult.data ?? [], deposit_rules: depositRulesResult.data ?? [] });
  } catch (error) {
