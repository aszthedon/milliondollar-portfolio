...
export async function POST(request: Request) {
  try {
    ...
    for (const addon of (selectedAddons ?? []) as AddonRow[]) {
      const id = Number(addon.id);
      const rawQuantity = requestedQuantities[String(id)] ?? 1;
      const minQuantity = Math.max(1, Number(addon.min_quantity ?? 1));
      const maxQuantity = Math.max(minQuantity, Number(addon.max_quantity ?? 1));
      const quantity = addon.allow_quantity ? Math.min(Math.max(Math.floor(rawQuantity), minQuantity), maxQuantity) : 1;
      addonQuantities[String(id)] = quantity;
    }
    ...
    const addonLabels = ((selectedAddons ?? []) as AddonRow[]).map((addon) => { const qty = addonQuantities[String(addon.id)] ?? 1; const qtyText = qty > 1 ? ` x${qty}` : ""; return `${addon.name}${qtyText} ($${(Number(addon.price ?? 0) * qty).toFixed(2)})`; });
