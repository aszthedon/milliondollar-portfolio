  ...
  try {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="admin-login-attempt-trends-${days}-days.csv"`,
      },
    });
  } catch (error) {
    console.error("EXPORT ADMIN LOGIN ATTEMPT ANALYTICS ERROR:", error);

    return NextResponse.json(
      {
        error: "Admin login attempt analytics could not be exported.",
      },
      {
        status: 500,
      }
