...
function variationPayload(input: VariationFormState | Variation) {
  return {
    service_id: Number(input.service_id ?? 0),
    variation_name: String(input.variation_name ?? "").trim(),
    price: Number(input.price ?? 0),
    duration: Number(input.duration ?? 60),
  };
}
...
export default function ServiceManagerPage() {
  ...
  async function saveAllSections() {
    try {
      ...
      setMessage("Service sections saved.");
      await loadAll();
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : "Service sections could not be saved.");
    } finally { setBusy(false); }
  }
  ...
  async function deleteSection(id: number) {
    try {
      ...
      setMessage("Service section deleted and services were moved to Unsectioned.");
      await loadAll();
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : "Service section could not be deleted.");
    } finally { setBusy(false); }
  }
  ...
  async function handleCreateVariation() {
    try {
      ...
      setVariationForm((current) => ({ ...blankVariationForm, service_id: current.service_id }));
        ...
        <div className="mx-auto grid max-w-6xl gap-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-zinc-400">Create service sections, assign services into sections, reorder sections, and manage pricing/quantity/recurring settings.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={saveAllServices} disabled={busy || loading || services.length === 0} className="rounded-full bg-white px-5 py-3 text-sm font-black text-black disabled:opacity-60">Save All Services</button>
              <button type="button" onClick={saveAllVariations} disabled={busy || loading || variations.length === 0} className="rounded-full border border-white/10 px-5 py-3 text-sm font-black text-white disabled:opacity-60">Save All Variations</button>
            </div>
          </div>
          ...
          {loading ? <div className="rounded-3xl border border-white/10 bg-white/5 p-8 text-zinc-400">Loading service manager...</div> : <>
            ...
            <section className="grid gap-5 rounded-3xl border border-white/10 bg-white/5 p-6">
              <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div><h2 className="text-2xl font-black">Edit Sections</h2><p className="mt-2 text-sm text-zinc-400">Rename sections, update descriptions, reorder them, or deactivate sections.</p></div>
              ...
              {sections.map((section) => <div key={section.id} className="grid gap-3 rounded-2xl border border-white/10 bg-black p-4 md:grid-cols-[1fr_1.5fr_0.4fr_0.4fr_0.4fr]">
                ...
                <Field label="Order" type="number" value={String(section.sort_order ?? 100)} onChange={(value) => updateSection(section.id, { sort_order: Number(value) })} />
            ...
            <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <Field label="Service Title" value={form.title} onChange={(value) => setForm((current) => ({ ...current, title: value }))} />
                <Select label="Section" value={form.section_id} onChange={(value) => setForm((current) => ({ ...current, section_id: value }))} options={[["", "Unsectioned"], ...sections.map((section) => [String(section.id), section.title] as [string, string])]} />
                <Field label="Price Per Unit" value={form.price} onChange={(value) => setForm((current) => ({ ...current, price: value }))} type="number" />
                <Field label="Duration Minutes Per Unit" value={form.duration} onChange={(value) => setForm((current) => ({ ...current, duration: value }))} type="number" />
                <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-black px-4 py-3"><input type="checkbox" checked={form.allow_quantity} onChange={(event) => setForm((current) => ({ ...current, allow_quantity: event.target.checked, max_quantity: event.target.checked ? current.max_quantity : "1" }))} /> Enable service quantity</label>
                {form.allow_quantity && <QuantityFields value={form} onChange={(updates) => setForm((current) => ({ ...current, ...updates }))} />}
                <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-black px-4 py-3"><input type="checkbox" checked={form.is_recurring} onChange={(event) => setForm((current) => ({ ...current, is_recurring: event.target.checked }))} /> Enable recurring service</label>
                {form.is_recurring && <RecurringFields value={form} onChange={(updates) => setForm((current) => ({ ...current, ...updates }))} />}
                <TextArea label="Description" value={form.description} onChange={(value) => setForm((current) => ({ ...current, description: value }))} />
              </div>
              <button type="button" onClick={handleCreate} disabled={busy} className="mt-5 rounded-full bg-white px-6 py-3 text-sm font-black text-black disabled:opacity-60">Create Service</button>
            </section>
            ...
            <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <Field label="Price" value={variationForm.price} onChange={(value) => setVariationForm((current) => ({ ...current, price: value }))} type="number" />
                <Field label="Duration Minutes" value={variationForm.duration} onChange={(value) => setVariationForm((current) => ({ ...current, duration: value }))} type="number" />
              </div>
            ...
            <section className="grid gap-6">
              <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 md:flex-row md:items-end md:justify-between">
                <div><h2 className="text-2xl font-black">Edit Services</h2><p className="mt-2 text-sm text-zinc-400">Change multiple services first, then save them all at once.</p></div>
                <div className="flex flex-wrap gap-3">
                  <button type="button" onClick={saveAllServices} disabled={busy || services.length === 0} className="rounded-full bg-white px-6 py-3 text-sm font-black text-black disabled:opacity-60">Save All Services</button>
                  <button type="button" onClick={saveAllVariations} disabled={busy || variations.length === 0} className="rounded-full border border-white/10 px-6 py-3 text-sm font-black text-white disabled:opacity-60">Save All Variations</button>
                </div>
              </div>
                  ...
                  {group.services.map((service) => {
                    ...
                    return <div key={service.id} className="rounded-2xl border border-white/10 bg-black p-4">
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <Select label="Section" value={String(service.section_id ?? "")} onChange={(value) => updateLocal(service.id, { section_id: value ? Number(value) : null })} options={[["", "Unsectioned"], ...sections.map((section) => [String(section.id), section.title] as [string, string])]} />
                        <Field label="Price Per Unit" value={String(service.price ?? "")} onChange={(value) => updateLocal(service.id, { price: Number(value) })} type="number" />
                        <Field label="Duration Per Unit" value={String(service.duration ?? "")} onChange={(value) => updateLocal(service.id, { duration: Number(value) })} type="number" />
                        <Field label="Listing Order" value={String(service.sort_order ?? 100)} onChange={(value) => updateLocal(service.id, { sort_order: Number(value) })} type="number" />
                        <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3"><input type="checkbox" checked={Boolean(service.allow_quantity)} onChange={(event) => updateLocal(service.id, { allow_quantity: event.target.checked, max_quantity: event.target.checked ? Math.max(Number(service.max_quantity ?? 1), 2) : 1 })} /> Enable quantity</label>
                        {service.allow_quantity && <QuantityFields value={{ min_quantity: String(service.min_quantity ?? 1), max_quantity: String(service.max_quantity ?? 1), quantity_label: service.quantity_label ?? "Quantity" }} onChange={(updates) => updateLocal(service.id, updates as Partial<Service>)} />}
                        <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3"><input type="checkbox" checked={Boolean(service.is_recurring)} onChange={(event) => updateLocal(service.id, { is_recurring: event.target.checked })} /> Enable recurring</label>
                        {service.is_recurring && <RecurringFields value={{ recurring_interval: service.recurring_interval ?? "weekly", recurring_count: String(service.recurring_count ?? 4), recurring_label: service.recurring_label ?? "Recurring service" }} onChange={(updates) => updateLocal(service.id, updates as Partial<Service>)} />}
                        <TextArea label="Description" value={service.description ?? ""} onChange={(value) => updateLocal(service.id, { description: value })} />
                      </div>
                      <div className="mt-4 flex flex-wrap gap-3">
                        <button onClick={() => saveService(service)} disabled={busy} className="rounded-full border border-green-500 px-5 py-2 text-green-400 disabled:opacity-50">Save This Service</button>
                        <button onClick={() => deleteService(service.id)} disabled={busy} className="rounded-full border border-red-500 px-5 py-2 text-red-400 disabled:opacity-50">Delete Service</button>
                      </div>
                      ...
                      {serviceVariations.length > 0 && <div className="mt-5 grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                        ...
                        {serviceVariations.map((variation) => <div key={variation.id} className="grid gap-3 rounded-xl border border-white/10 bg-black p-3 md:grid-cols-[1fr_0.4fr_0.4fr_auto]">
                          <Field label="Name" value={variation.variation_name ?? ""} onChange={(value) => updateLocalVariation(variation.id, { variation_name: value })} />
                          <Field label="Price" type="number" value={String(variation.price ?? "")} onChange={(value) => updateLocalVariation(variation.id, { price: Number(value) })} />
                          <Field label="Duration" type="number" value={String(variation.duration ?? "")} onChange={(value) => updateLocalVariation(variation.id, { duration: Number(value) })} />
                          <div className="flex items-end gap-2"><button onClick={() => saveVariation(variation)} disabled={busy} className="rounded-full border border-green-500 px-4 py-2 text-sm text-green-400 disabled:opacity-50">Save</button><button onClick={() => deleteVariation(variation.id)} disabled={busy} className="rounded-full border border-red-500 px-4 py-2 text-sm text-red-400 disabled:opacity-50">Delete</button></div>
                        </div>)}
                      </div>}
                    </div>;
                  })}
...
function QuantityFields({ value, onChange }: { value: { min_quantity?: string | number | null; max_quantity?: string | number | null; quantity_label?: string | null }; onChange: (updates: Partial<FormState>) => void }) {
  return <div className="grid gap-4 rounded-2xl border border-white/10 bg-black p-4 md:col-span-2 md:grid-cols-3"><Field label="Minimum Quantity" value={String(value.min_quantity ?? 1)} onChange={(next) => onChange({ min_quantity: next })} type="number" /><Field label="Maximum Quantity" value={String(value.max_quantity ?? 1)} onChange={(next) => onChange({ max_quantity: next })} type="number" /><Field label="Quantity Label" value={String(value.quantity_label ?? "Quantity")} onChange={(next) => onChange({ quantity_label: next })} /></div>;
}
